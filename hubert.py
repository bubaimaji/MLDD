...
# -------------------- CONFIG --------------------
MODEL_NAME = "facebook/hubert-large-ls960-ft"  # change to large if needed
...
hidden_size = getattr(model.config, "hidden_size", 104)
...
def extract_segment_feature(filepath):
    ...
    with torch.no_grad():
        ...
        # Use mid-to-late layers (6–11 for base, 12–17 for large)
        if hidden_states.shape[0] >= 12:
            selected = hidden_states[6:12]
        else:
...
print(f"\n Speaker-level HuBERT feature extraction complete for {SPLIT} set.")
