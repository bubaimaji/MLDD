...
FEAT_DIR_B = "/home/bubai-maji/bubai/English/features_npy/wavlm_base"  
SPLIT_TRAIN = "train"
SPLIT_DEV   = "dev"
...
if __name__ == "__main__":
    ...
    for r in range(RUNS):
        print(f"\n==================== RUN {r+1}/{RUNS} ====================\n")
        metrics = train_and_eval()  # this returns dict

        all_metrics["acc"].append(metrics["acc"])
        all_metrics["prec"].append(metrics["prec"])
        all_metrics["rec"].append(metrics["rec"])
        all_metrics["f1"].append(metrics["f1"])
        all_metrics["wf1"].append(metrics["wf1"])
        all_metrics["auc"].append(metrics["auc"])
        all_metrics["ua"].append(metrics["ua"])

    # ===== FINAL AGGREGATED RESULTS =====
    print("\n==================== FINAL RESULTS OVER RUNS ====================")
    for key, values in all_metrics.items():
        mean = np.mean(values)
        std  = np.std(values)
        print(f"{key.upper():5s}  Mean={mean:.4f}   STD={std:.4f}")
