...
def extract_segment_feature(filepath):
    """Extract multi-layer pooled Whisper encoder embedding."""
    ...
    # --- FIX HERE: pad or trim to exactly 30 seconds (480,000 samples) ---
    max_len = 480000
    num_samples = waveform.shape[1]
    if num_samples < max_len:
        pad_len = max_len - num_samples
        waveform = torch.nn.functional.pad(waveform, (0, pad_len))
    elif num_samples > max_len:
        waveform = waveform[:, :max_len]
    # ---------------------------------------------------------------------

    # Feature extraction (Whisper needs exactly 3000 mel frames)
    ...
    with torch.no_grad():
        ...
        hidden_states = torch.stack(enc_out.hidden_states)  # (num_layers, 1, T, D)
        mean_hidden = hidden_states[-4:].mean(0).squeeze(0)  # last 4 layers averaged

        # Statistical pooling (mean + std)
        mean_vec = mean_hidden.mean(dim=0)
        std_vec = mean_hidden.std(dim=0)
        pooled = torch.cat([mean_vec, std_vec])
        #pooled = torch.cat([mean_vec])
...
for idx, pid in enumerate(speaker_features.keys()):
    ...
    pooled_feat = feats.mean(axis=0)         # mean pooling across segments
...
print(f"\n Speaker-level Whisper feature extraction complete for {SPLIT} set.")
