...
OUTPUT_DIR = f"{BASE_DIR}/English/edic_features_npy/hubert_base/{SPLIT}"
...
print("\n Loaded pretrained HuBERT-Large\n")
...
def extract_hubert_features(audio_path):
    ...
    with torch.no_grad():
        ...
        hidden_states = outputs.last_hidden_state  # shape: (1, T, 1024)

    # Mean pool across time → shape: (1024,)
...
for i, row in tqdm(df.iterrows(), total=len(df)):
    ...
    try:
        feat = extract_hubert_features(seg_path)
...
print("\n Segment-level HuBERT feature extraction DONE!")
print(f"Output dir: {OUTPUT_DIR}")
print(f"Embeddings shape: {segment_feats.shape}  (segments × 1024)")
