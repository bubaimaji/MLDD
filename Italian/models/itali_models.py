...
def train_fold(fold_idx):
    ...
    for m, base in enumerate(BASE_DIRS):
        ...
        X_list, y_list, utt_list, spk_list = [], [], [], []

        for f in range(1, FOLDS+1):
            Xf, yf, ut, sp = load_fold(base, f)
            X_list.append(Xf)
            y_list.append(yf)
            utt_list.append(ut)
            spk_list.append(sp)
...
plot_tsne(initial_emb, final_emb, labels, "results/tsne.png")

plot_auc_curve_folds(FOLD_FPRS, FOLD_TPRS, FOLD_AUCS, "results/roc.png")
...
plot_confusion_matrix(all_true, pred, "results/confusion.png")

print("\nSaved TSNE, ROC, Confusion Matrix in results/")
