    ...
    for fld in df[col].dropna().astype(str).str.strip("'"):
        ...
        for fname in os.listdir(root):
            ...
            for i, start in enumerate(range(0, duration_ms, 4000)):
                ...
                # ---------------------------------------
                # Extract speaker_id and utterance_id
                # ---------------------------------------
                base = os.path.basename(seg_name)
                utterance_id = base.split("_seg")[0]           # e.g. 01_CF56_1_01_CF56_1_7
                speaker_id = "_".join(utterance_id.split("_")[:3])  # e.g. 01_CF56_1
...
seg_df.to_csv("speaker_metadata.csv", index=False)
print(f"Saved {len(seg_df)} segments with speaker_id and utterance_id columns.")
