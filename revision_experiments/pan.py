...
class PanCrossAttention(nn.Module):

    def __init__(self, dims, latent=256, dropout=0.3):
        super().__init__()

        assert len(dims) == 2

        # projections
        self.proj_speech = ModalityProj(dims[0], latent)
        self.proj_wavlm  = ModalityProj(dims[1], latent)

        # Q K V
        self.q_speech = nn.Linear(latent, latent)
        self.k_wavlm  = nn.Linear(latent, latent)
        self.v_wavlm  = nn.Linear(latent, latent)

        self.q_wavlm  = nn.Linear(latent, latent)
        self.k_speech = nn.Linear(latent, latent)
        self.v_speech = nn.Linear(latent, latent)

        self.scale = latent ** 0.5

        # classifier (Pan paper style)
    ...
    def _fuse(self, Xs):
        ...
        speech = self.proj_speech(Xs[0])
        wavlm  = self.proj_wavlm(Xs[1])

        # speech attends wavlm
        q1 = self.q_speech(speech)
        k1 = self.k_wavlm(wavlm)
        v1 = self.v_wavlm(wavlm)

        score1 = torch.sigmoid(
            (q1 * k1).sum(dim=1, keepdim=True) / self.scale
        )

        speech_out = score1 * v1

        # wavlm attends speech
        q2 = self.q_wavlm(wavlm)
        k2 = self.k_speech(speech)
        v2 = self.v_speech(speech)

        score2 = torch.sigmoid(
            (q2 * k2).sum(dim=1, keepdim=True) / self.scale
        )

        wavlm_out = score2 * v2

        fused = torch.cat(
            [speech_out, wavlm_out],
            dim=1
        )
...
def train_fold(fold_idx):
    ...
    model = PanCrossAttention(dims).to(DEVICE)
