...
for f in range(FOLDS):
    ...
    acc, prec, sens, spec, f1, wf1, auc, ua = train_fold(f)
    ...
    for key, val in zip(results.keys(), [acc, prec, sens, spec, f1, wf1, auc, ua]):
...
X_raw = np.vstack(ALL_INITIAL)
X_fusion = np.vstack(ALL_FINAL)
...
# -----------------------------------------------------
# Split IS10 and WavLM-B
# -----------------------------------------------------

IS10_DIM = 1582
WAVLM_DIM = 768

X_is10 = X_raw[:, :IS10_DIM]
X_wavlm = X_raw[:, IS10_DIM:IS10_DIM + WAVLM_DIM]
...
Z_is10 = tsne.fit_transform(X_is10)
...
Z_wavlm = tsne.fit_transform(X_wavlm)
...
Z_fusion = tsne.fit_transform(X_fusion)
