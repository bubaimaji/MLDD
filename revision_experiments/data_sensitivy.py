...
BASE_DIRS = [
    ...
    "/home/bubai-maji/bubai/Itali/features_npy/microsoft-wavlm-base",
]
...
def reduce_training_data(X_list, y, spk, ratio, seed=42):

    if ratio == 1.0:
        return X_list, y, spk

    # -----------------------------------
    # Get one label per speaker
    # -----------------------------------
    ...
    for s in unique_spk:
        speaker_labels.append(
            y[np.where(spk == s)[0][0]]
        )

    speaker_labels = np.array(speaker_labels)

    # -----------------------------------
    # Split speakers (NOT segments)
    # -----------------------------------
    ...
    # -----------------------------------
    # Keep all segments from those speakers
    # -----------------------------------
    mask = np.isin(spk, keep_spk)

    X_new = [X[mask] for X in X_list]
    y_new = y[mask]
    spk_new = spk[mask]

    print(
    f"Training speakers: "
    f"{len(unique_spk)} -> {len(keep_spk)}"
    )
    ...
    return X_new, y_new, spk_new

# =====================================================
# TRAIN ONE FOLD
# =====================================================
def train_fold(fold_idx, train_ratio=1.0):
    ...
    Xtr_scaled, ytrain, spk_train = reduce_training_data(
        Xtr_scaled,
        ytrain,
        spk_train,
        train_ratio,
...
for ratio in TRAIN_RATIOS:
    ...
    for f in range(FOLDS):
        ...
        acc, prec, sens, spec, f1, wf1, auc, ua = train_fold(
            f,
            train_ratio=ratio
        )
        ...
        for key, val in zip(
            results.keys(),
            [acc, prec, sens, spec, f1, wf1, auc, ua]
        ):
            results[key].append(val)

    print("\n")
    print(f"RESULTS FOR {int(ratio*100)}% TRAINING DATA")
