...
class IliasCrossAttentionFusion(nn.Module):

    def __init__(self, dims, latent=768, dropout=0.4):
        super().__init__()

        assert len(dims) == 2

        # project IS10 and WavLM to same dimension
        ...
        # ---------- TEXT BRANCH ----------
        self.q_t = nn.Linear(latent, latent)
        self.k_t = nn.Linear(latent, latent)
        self.v_t = nn.Linear(latent, latent)

        # ---------- SPEECH BRANCH ----------
        self.q_s = nn.Linear(latent, latent)
        self.k_s = nn.Linear(latent, latent)
        self.v_s = nn.Linear(latent, latent)

        self.norm1_t = nn.LayerNorm(latent)
        self.norm1_s = nn.LayerNorm(latent)

        # shared FFN (paper: d -> 4d -> d)
        ...
        self.norm2_t = nn.LayerNorm(latent)
        self.norm2_s = nn.LayerNorm(latent)

        # classifier
        self.classifier = nn.Sequential(
            nn.Dropout(0.4),
            nn.Linear(4 * latent, 128),
            nn.ReLU(),
            nn.Dropout(0.4),
    ...
    def _cross_attention_scaling(self, speech, wavlm):
        ...
        St = torch.sigmoid(
            torch.sum(Qi * Kt, dim=1, keepdim=True) / self.scale
        )
        ...
        Si = torch.sigmoid(
            torch.sum(Qt * Ki, dim=1, keepdim=True) / self.scale
        )

        wavlm_att = Si * Vi

        return speech_att, wavlm_att

    def _fuse(self, Xs):

        speech = self.proj[0](Xs[0])
        wavlm  = self.proj[1](Xs[1])

        # Cross-Attention Scaling
        speech_att, wavlm_att = self._cross_attention_scaling(
            speech,
            wavlm
        )

        # Add + LayerNorm
        speech_hat = self.norm1_t(speech_att + speech)
        wavlm_hat  = self.norm1_s(wavlm_att + wavlm)

        # Shared FFN
        speech_ff = self.shared_ffn(speech_hat)
        wavlm_ff  = self.shared_ffn(wavlm_hat)

        # LayerNorm
        speech_prime = self.norm2_t(speech_ff)
        wavlm_prime  = self.norm2_s(wavlm_ff)

        # paper concatenation:
        # [E_hat , E_prime]
        speech_final = torch.cat(
            [speech_hat, speech_prime],
            dim=1
        )

        wavlm_final = torch.cat(
            [wavlm_hat, wavlm_prime],
            dim=1
        )

        # final fusion
        fused = torch.cat(
            [speech_final, wavlm_final],
            dim=1
        )
...
def train_fold(fold_idx):
    ...
    model = IliasCrossAttentionFusion(dims).to(DEVICE)
...
for f in range(FOLDS):
    ...
    print(f"\n FOLD {f+1} RESULTS (Speaker-level)")
    print(f"ACC  : {acc:.4f}")
